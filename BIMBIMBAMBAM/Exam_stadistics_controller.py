...
class statsController:
    ...
    def get_exam_statistics(self, id_examen):
        ...
        promedio = sum(calificacion[1] for calificacion in calificaciones) / len(calificaciones) if calificaciones else 0
    ...
    def get_student_results(self, id_examen, id_estudiante):
        ...
        query_resultados = (
            "SELECT e.id_estudiante, MAX(e.calificacion) AS calificacion, "
    ...
    def insert_statistics(self, id_results, student_results, id_ex, results, student_id):
        # Insertar estadísticas en la tabla statistics
        query_insert_statistics = (
            "INSERT INTO statistics (id_results, student_results, id_ex, results, student_id) "
            "VALUES (%s, %s, %s, %s, %s)"
        )
        self.cursor.execute(query_insert_statistics, (id_results, student_results, id_ex, results, student_id))
        self.db.commit()
        print("Estadísticas insertadas con éxito")